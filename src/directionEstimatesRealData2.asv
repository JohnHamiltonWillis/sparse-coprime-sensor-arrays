function [pMSE,mMSE,dMSE,fMSE] = directionEstimatesRealData(M, N, U1, U2,SampleRange, deg)
    ...
    for i = 1:num   
        ...
        totalData = totalData(RealSampleSize,:);
        size(totalData)
        x = x + totalData;
    end
   ...
   %Sets the range of samples to be used
    
    %x = totalData1(RealSampleSize,:)+totalData2(RealSampleSize,:); %The field data within the sample range is called to x
    ...
    if plot_fig
        ...
        for idx = 1:size(us,1)
            hold on;
            plot([us(idx,1) us(idx,2)],[lowerlimit 0],'r:','LineWidth',2);
        end   
        legend('Product','Min','Direct','Full ULA');
        for i = 1:size(us,1)
            legend(['Actual u_' num2str(i)])
        end
        hold on;
