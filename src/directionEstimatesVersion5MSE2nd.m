function [dMSE,dpMSE,flags]= directionEstimatesVersion5MSE(SNRdB,SampleSize)
    ...
    while flag && counter < 100%%%%%If the data set is not good, we need to discard the data set and come back here
        ...
        for kdx = 1:SampleSize
            ...
            rnew(3,kdx) = xtotal(4,kdx)*conj(xtotal(6,kdx));
            rnew(4,kdx) = xtotal(6,kdx)*conj(xtotal(9,kdx));
            rnew(5,kdx) = xtotal(4,kdx)*conj(xtotal(8,kdx));
            rnew(6,kdx) = xtotal(4,kdx)*conj(xtotal(9,kdx));
            rnew(7,kdx) = xtotal(3,kdx)*conj(xtotal(9,kdx));
            rnew(8,kdx) = xtotal(8,kdx)*conj(xtotal(15,kdx));
            rnew(9,kdx) = xtotal(4,kdx)*conj(xtotal(12,kdx));
            rnew(10,kdx) = xtotal(3,kdx)*conj(xtotal(12,kdx));
            rnew(11,kdx) = xtotal(6,kdx)*conj(xtotal(16,kdx));
            rnew(12,kdx) = xtotal(1,kdx)*conj(xtotal(12,kdx));
            rnew(13,kdx) = xtotal(4,kdx)*conj(xtotal(16,kdx));
            rnew(14,kdx) = xtotal(3,kdx)*conj(xtotal(16,kdx));
            rnew(15,kdx) = xtotal(4,kdx)*conj(xtotal(18,kdx));
            rnew(16,kdx) = xtotal(3,kdx)*conj(xtotal(18,kdx));
            rnew(17,kdx) = xtotal(4,kdx)*conj(xtotal(20,kdx));
            rnew(18,kdx) = xtotal(3,kdx)*conj(xtotal(20,kdx));
            rnew(19,kdx) = xtotal(3,kdx)*conj(xtotal(21,kdx));
            rnew(20,kdx) = xtotal(1,kdx)*conj(xtotal(12,kdx));
            rnew(21,kdx) = xtotal(1,kdx)*conj(xtotal(12,kdx));
            rnew(22,kdx) = xtotal(1,kdx)*conj(xtotal(12,kdx));
            tempR = conv(dataset.',fliplr(conj(dataset.')));
            tempR(1:temp) = [];
            if ~isempty(firstzeroindex)
                tempR(firstzeroindex:end) = [];
